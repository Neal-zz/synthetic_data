...
class HeapStateSpace(gym.Space):
    ...
    def __init__(self, physics_engine, config):
        ...
        self.max_objs = config["max_objs"]  # 10
        self.min_objs = 1
        if "min_objs" in config.keys():
            self.min_objs = config["min_objs"]  # 2
        self.replace = config["replace"]  # 0

        self.max_obj_diam = config["max_obj_diam"]  # 0.3
        self.drop_height = config["drop_height"]  # 0.2
        self.max_settle_steps = config["max_settle_steps"]  # 500
        self.mag_v_thresh = config["mag_v_thresh"]  # 0.005
        self.mag_w_thresh = config["mag_w_thresh"]  # 0.1
        ...
        self.obj_density = obj_config["density"]  # 4000
        ...
        for root, _, files in os.walk(self._mesh_dir):
            ...
            if dataset_name in obj_config["object_keys"].keys():  # ycb
                for f in files:
    ...
    def sample(self):
        ...
        workspace_objs = self._config["workspace"]["objects"]  # bin & plane
        ...
        num_objs = min(num_objs, self.max_objs)  # <=10
        num_objs = max(num_objs, self.min_objs)  # >=2
        ...
        while total_drops < 2 * num_objs and len(objs_in_heap) < num_objs:
            ...
            self._logger.info(obj_state_key)  # ycb~070-a_colored_wood_blocks~0

            # if the radius is too big, give up this drop.
            ...
            delta_com = self.delta_com_rv.rvs(size=1)  # [x,y,z] 数量级约为 1e-7
            center_of_mass = obj.mesh.center_mass + delta_com
            obj.mesh.center_mass = center_of_mass

            # sample obj drop pose
            obj_orientation = self.obj_orientation_space.sample()  # 2
            ...
            R_obj_drop_world = R_table_world.dot(T_obj_table.rotation)  # w^R_t * t^R_o
            ...
            # simulate until approx zero velocity
            wait = time.time()
            objects_in_motion = True
            num_steps = 0
            while objects_in_motion and num_steps < self.max_settle_steps:  # < 500
                ...
                max_mag_w = 0
                objs_to_remove = set()
                for o in objs_in_heap:  # 可能引起场景中已有物体的运动？
                    try:
                ...
                if (
                    max_mag_v < self.mag_v_thresh  # 0.005
                    and max_mag_w < self.mag_w_thresh  # 0.1
                ):
            ...
            for o in objs_in_heap:
                ...
                if not self.in_workspace(obj_pose):  # [-0.2,-0.25,0.0] ~ [0.2,0.25,0.3]
                    self._logger.warning(
